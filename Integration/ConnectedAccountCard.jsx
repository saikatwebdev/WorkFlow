  ...
  return (
    <div className="group bg-white rounded-2xl p-6 shadow-sm border border-slate-200 hover:shadow-xl transition-all duration-500 hover:scale-[1.02] hover:border-blue-300 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-32 h-32 opacity-5 transition-opacity duration-500 group-hover:opacity-10">
        <div className="w-full h-full bg-gradient-to-br from-green-400 to-blue-400 rounded-full blur-3xl transform rotate-45"></div>
      </div>
      
      <div className="relative flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <div className={`w-14 h-14 rounded-2xl ${getPlatformGradient(account.platform)} flex items-center justify-center shadow-lg transform transition-all duration-300 group-hover:scale-110`}>
            {getPlatformIcon(account.platform)}
          </div>
          <div>
            <h3 className="font-bold text-slate-900 text-lg">{account.platform}</h3>
        ...
        <div className="flex items-center space-x-3">
          {account.connected ? (
            <>
              <span className="px-4 py-2 bg-emerald-100 text-emerald-800 text-xs font-semibold rounded-full border border-emerald-200">
                Connected
              </span>
              <button 
                onClick={() => handleDisconnect(account.id)}
                className="text-red-400 hover:text-red-600 p-2 rounded-xl hover:bg-red-50 transition-all duration-300 hover:scale-110"
                title="Disconnect"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <button className="px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-sm font-semibold rounded-xl hover:from-blue-600 hover:to-blue-700 transition-all duration-300 transform hover:scale-105 shadow-lg">
              Connect
            </button>
          )}
        </div>
